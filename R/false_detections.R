...
#' value to impute.
#' @param run_seq: If TRUE, run sequentially instead of parallelizing.
#'
...
get_false_detection <- function(positions, test_result, link_d,use_link_d = TRUE, 
                                ...
                                min_pr = 0,  keep_null_tests = FALSE, run_seq = FALSE){
  ...
  cat <- catalogue(x = positions$x, y = positions$y,test_result = test_result, link_d=link_d, 
                   ...
                   min_pos=min_pos, min_total=min_total, min_pr=min_pr, 
                   keep_null_tests=keep_null_tests, in_latlon=FALSE, to_epsg = NULL, 
                   verbose = FALSE)
  cat_real <- data.table(
...
#' Generate random distributions based on the global prevalence of infections and 
#' compute its Epifriends to undermine false positive detections.
#'
#' @param positions data.frame with the positions of parameters we want to query 
#' with shape (n,2) where n is the number of positions.
#' @param test_result data.frame with the test results (0 or 1).
...
#' @param run_seq: If TRUE, run sequentially instead of parallelizing.
#'
#' @return  data.frame with the number of isolated positives detected per iteration 
#' and its frequency.
#'   
...
#'
#' # Creation of catalogue for this positions, linking distance 2 and default values.
#' cat <- generate_simulations(positions = pos, test_result = test, link_d = 2,
#'  n_simulations = 2, run_seq=TRUE)
#' 
generate_simulations <- function(positions, test_result, link_d, use_link_d = TRUE, 
  ...
  scenarios <- foreach(i = 1:n_simulations) %dopar% {
    df[, test := rbinom(nrow(df), 1, prevalence)]
