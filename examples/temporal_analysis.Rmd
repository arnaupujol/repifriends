...
while(min_date + time_steps*step_num + time_width <= max_date){
  ...
  for(epi in 1:length(epi_catalogue_list$temporal_catalogues[[step_num+1]]$id)){
    ...
    vmin <- 0
    vmax <- max(epi_catalogue_list$temporal_catalogues[[step_num+1]]$id)
...
```{r}
selected_data <- (dates >= min_date + time_steps*0)&
                    (dates <= min_date + time_steps0 + time_width)

indeces <- epi_catalogue_list$temporal_catalogues[[1]]$indeces[[1]]
indeces
vmin <- 0
vmax <- max(epi_catalogue_list$temporal_catalogues[[1]]$id)
position_rand[selected_data,][indeces,]

graph <- ggplot( mock_rand[selected_data,][indeces,],aes(x=x, y=y)) +
  geom_point(aes(colour = as.factor(test)), size = 2.5, shape = 21, fill = as.integer(epi_catalogue_list$temporal_catalogues[[1]]$id[[1]]+2))
graph
```
