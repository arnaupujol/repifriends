...
scatter_pval <- function(coordinates, id_data, positive, epi_catalogue){
    ...
    graph <- ggplot(coordinates,aes(x=x, y=y))+
            ...
            geom_point(data = pos[id_data >0,], aes(colour = p_vals))+
...
size_histogram <- function(catalogue){
  ...
  # and p>0.05 in blue.
  histo <- ggplot(cat$epifriends_catalogue,aes(x=total))+
          geom_histogram(colors = "blue")
}
```

## Create mock data 

The data created with the epifriends_on_different_distributions.py file of python and saved on this R project is readed:
...
graph <- ggplot(neg,aes(x=x, y=y))+
            geom_point(color = "grey")+
            geom_point(data = pos, aes(colour = cluster_id))+
