...
```{r}
plot1 <- scatter_pval(position, cat$cluster_id, positive, cat$epifriends_catalogue)
plot1
```
...
```{r}
catalogues <- catalogue(position, test_result, link_d, cluster_id = NULL,
...
```{r}
plot2 <- scatter_pval(position, catalogues$cluster_id, positive, catalogues$epifriends_catalogue, title = "P-value after removing False Detections")
plot2
```
