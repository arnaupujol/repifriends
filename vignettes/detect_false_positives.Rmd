...
n_simulations = 100
scenarios <- generate_simulations(positions = position, test_result = test_result$test, 
...
```{r}
cat <- catalogue(position, test_result$test, link_d, cluster_id = NULL, min_neighbours = min_neighbours)
cat_real <- data.table(
...
```{r}
plot1 <- scatter_pval(position, cat$cluster_id, positive, prevalence = NULL, cat$epifriends_catalogue)
plot1
```
...
```{r}
catalogues <- catalogue(position, test_result$test, link_d, cluster_id = NULL,
...
```{r}
plot2 <- scatter_pval(position, catalogues$cluster_id, positive, prevalence = NULL, catalogues$epifriends_catalogue, title = "P-value after removing False Detections")
plot2
```