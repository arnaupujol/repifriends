...
```{r, warning=FALSE}
setwd("./../")
mock_rand <- data.table(read.csv("./data/mock_data_rand_temp.csv"))
...
```{r}
epi_catalogue_list <- temporal_catalogue(position_rand, test_rand, mock_rand$date, link_d,prevalence, min_neighbours, time_width, min_date, max_date, time_steps, add_temporal_id = FALSE)
```

```{r}
epi_catalogue_list <- temporal_catalogue(position_rand, test_rand, mock_rand$date, link_d,prevalence, min_neighbours, time_width, min_date, max_date, time_steps, add_temporal_id = TRUE, linking_time = 3, linking_dist = 0.15, get_timelife = TRUE)
```
...
#Get temporal IDs
epi_catalogue_list_id <- temporal_catalogue(position_rand, test_rand, mock_rand$date, link_d, prevalence, min_neighbours, time_width, min_date, max_date, time_steps, add_temporal_id = TRUE, linking_time = linking_time, linking_dist = linking_dist, get_timelife = TRUE)
```
